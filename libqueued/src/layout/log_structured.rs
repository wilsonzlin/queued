...
use crate::buffered_read::SeekableAsyncFileBuffer;
...
impl StorageLayout for LogStructuredLayout {
  ...
  async fn load_data_from_device(&self, metrics: Arc<Metrics>) -> LoadedData {
    ...
    let (head, tail) = self.log_structured.get_head_and_tail();

    let mut virtual_offset = head;
    // Reading millions of tiny messages serially is not fast, so use a buffered reader.
    let mut bufread = SeekableAsyncFileBuffer::new(
      self.device.clone(),
      self.device_size,
      1024 * 1024 * 1024, // TODO Allow configuration.
    );
    while virtual_offset < tail {
      let physical_offset = self.log_structured.physical_offset(virtual_offset);
      let typ = LogEntryType::try_from(bufread.read_at(physical_offset, 1).await[0]).unwrap();
      match typ {
        LogEntryType::DummyFiller => {
        ...
        LogEntryType::Poll => {
          let raw = bufread.read_at(physical_offset, LOGENT_POLL_SIZE).await;
          let id = raw.read_u64_be_at(LOGENT_POLL_OFFSETOF_ID);
        ...
        LogEntryType::Update => {
          let raw = bufread.read_at(physical_offset, LOGENT_UPDATE_SIZE).await;
        ...
        LogEntryType::Delete => {
          let raw = bufread.read_at(physical_offset, LOGENT_DELETE_SIZE).await;
